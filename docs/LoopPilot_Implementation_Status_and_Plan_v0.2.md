...
- 已有 runtime JSON Schema、schema drift、wrapper parity、scan output、scan secret-safety、export template、fixture coverage taxonomy minimums、export command、save command、install/doctor integration、doctor JSON output 验证。
...
| fixtures | 至少 45 条，三类各至少 15 条 | 已完成 | `.looppilot/fixtures/decision-fixtures.jsonl` 当前有 45 行，validator 允许后续扩展 | v0 完成 |
...
- `report-fixture-coverage.mjs` 统计 decision 分布、风险关键词覆盖、细分 taxonomy 最低覆盖，并防止高风险 fixture 进入 `RUN_WITH_CONTRACT`。
...
3. doctor 已支持 `--json --output <path>`；后续可选增强为更多 CI metadata。
...
**当前已补齐本地 runtime JSON Schema validation、schema drift、wrapper parity、fixture coverage taxonomy minimums、只读且报告敏感路径的 scan helper、scan secret-safety、显式 export fallback、显式 save commands、install/doctor integration、doctor JSON output 与 report template；后续重点是保持安全门禁稳定，并继续坚持 agent-native、chat-first、no runner、no provider、no automatic commit/push/deploy。**
