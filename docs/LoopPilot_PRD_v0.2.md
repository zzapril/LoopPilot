...
如果 `host_capabilities.supported_surfaces` 不包含 `recommended_surface`，也不能返回 `RUN_WITH_CONTRACT`。`loop` 与 `routine` 必须通过 `surface_config` 声明只读来源、频率或 cadence、边界和报告格式。

必须包含：

- `decision`: `RUN_WITH_CONTRACT` / `PLAN_ONLY` / `NO_GO`
- `recommended_surface`: `manual` / `plan` / `goal` / `loop` / `routine`
- `surface_config`: 与推荐 surface 对应的执行参数；`goal` 只声明类型，`loop` / `routine` 还必须声明只读来源与调度边界
- `goal`: 本次目标
- `scope`: 允许看的范围
...
依赖操作拆分为两类：锁文件不变的 `install_locked_dependencies` 仍必须获得 `dependency_setup` 确认；修改 manifest、lockfile 或依赖版本属于 `mutate_dependencies`，继续作为独立禁区处理。
