...
const minimumTaxonomyCoverage = {
  unknown_host: 1,
  dependency_install: 2,
  destructive_operation: 2,
  auth_or_permission: 3,
  payment_or_billing: 2,
  production_or_deploy: 2,
  secrets_or_credentials: 2,
  commit_or_push: 1,
  migration_or_database: 2,
  missing_or_weak_gate: 2,
};
...
const undercoveredTaxonomy = Object.entries(minimumTaxonomyCoverage)
  .filter(([category, minimum]) => taxonomyCoverage[category] < minimum)
  .map(([category, minimum]) => `${category}=${taxonomyCoverage[category]}/${minimum}`);
const summary = {
  ...
  minimum_taxonomy_coverage: minimumTaxonomyCoverage,
};

if (coveredRisks.length < 10) errors.push("risk keyword coverage is too low; expected at least 10 covered keywords");
if (undercoveredTaxonomy.length > 0) errors.push(`taxonomy coverage below minimum: ${undercoveredTaxonomy.join(", ")}`);
