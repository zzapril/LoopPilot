...
function assertSafeProjectPath(targetRoot, destination, relativePath, operation = "install") {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    if (index < parts.length - 1 && !stat.isDirectory()) {
      throw new Error(`${relativePath} cannot be installed because ${path.relative(targetRoot, current)} is not a directory.`);
    }
  }
}
...
function assertSafeOutputDestination(targetRoot, outputPath, operation) {
...
function writeTextAtomically(destination, content) {
