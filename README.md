...
Doctor checks installed Agent Pack files, fixtures, runtime JSON Schema compatibility, schema drift, wrapper references, and wrapper parity, then prints a per-check pass/fail report. Use `--json --output <path>` to explicitly save a machine-readable doctor report.
...
This validates at least 45 decision fixtures, at least 15 examples per decision type, and confirms that runtime JSON Schema checks, schema drift, wrappers, wrapper parity, scan helper output, scan secret-safety, export templates, fixture coverage taxonomy minimums, export command behavior, explicit save commands, and install/doctor integration satisfy the current safety gates.
